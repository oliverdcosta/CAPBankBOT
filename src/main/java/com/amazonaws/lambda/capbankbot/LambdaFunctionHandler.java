...
public class LambdaFunctionHandler implements RequestHandler<Map<String, Object>, Object> {
	...
	public Object handleRequest(Map<String, Object> input, Context context) {
		...
		DialogAction dialogAction = new DialogAction("Close", "Fulfilled",
				new Message("PlainText", "Sorry I did not understand what you said. For more information, please visit https://www.cap-bank.us.!"));
				...
				if (!(filltypeintent.equalsIgnoreCase("null"))) {
					if (!(filltypeintent.toLowerCase().indexOf("online") == -1)) {
						...
						String formLinkMessage = "Please complete the application here – " + formUrl
								+ " Hope I was able to help , " + firstName
								+ ". Please visit us at www.cap-bank.us for all your banking needs. ";
					...
					if (!(usercardintent.equalsIgnoreCase("null"))) {
						...
						if (!(featureIntent.equalsIgnoreCase("null"))) {
							...
							String featureIntentArg = "Cap Bank";
							// Service
							if(featureIntent.equalsIgnoreCase("cashrewards")) {
								featureIntentArg = "Cash Rewards";
							}
							if(featureIntent.equalsIgnoreCase("hotelrewards")) {
								featureIntentArg = "Hotel Rewards";
							}
							if(featureIntent.equalsIgnoreCase("travelmiles")) {
								featureIntentArg = "Travel Miles";
							}
...
//															"]}"));
			...
			dialogAction = new DialogAction("Close", "Fulfilled", new Message("PlainText",
					"Sorry I did not understand.This service is only to provide information about credit cards we offer and help you apply for a credit card; For more information, please visit https://www.cap-bank.us. We are here to help if you need support"));
			return new LexResponse(dialogAction);
		}
	...
	private ResponseCard processFeatureCard(String userscore, String firstName) {
		String scoreMessage = "Thanks; Are you looking for any specific features?";

		Button buttonArray[] = new Button[3];
		Button button = null;
		button = new Button("Travel Miles", "travelmiles");
		buttonArray[0] = button;
		button = new Button("Cash Rewards", "cashrewards");
		buttonArray[1] = button;
		button = new Button("Hotel Rewards", "hotelrewards");
		buttonArray[2] = button;
		Attachment attachmentArray[] = new Attachment[1];
		attachmentArray[0] = new Attachment(buttonArray, "Available Features", "Select any feature");
		ResponseCard responseCard = new ResponseCard(attachmentArray, 1, "application/vnd.amazonaws.card.generic");
		return responseCard;
	}
	...
	// card = (Card) cardList.get(i);
	// if(feature != null) {
	// button = new Button(card.getCard() +"-"+ feature,
	// card.getCard().toLowerCase().replaceAll("\\s", ""));
	// buttonArray[0] = button;
	// }else {
	// button = new Button(card.getCard(),
	// card.getCard().toLowerCase().replaceAll("\\s", ""));
	// buttonArray[0] = button;
	// }
	// //attachment = new Attachment(buttonArray, card.getCard(), "Browse and
	// Select", card.getCardThumbnail(),"");
	// attachment = new Attachment(buttonArray, card.getCard(), "Browse and
	// Select");
	// attachmentArray[i] = attachment;
	// }
	// ResponseCard responseCard = new ResponseCard(attachmentArray, 1,
